...
using namespace std;
    ...
    istream&  operator >>(istream& one,  Student& two){
      ...
      ostream& operator <<(ostream& one , const Student& two){
        one<<two.firstName<<'|'<<two.lastName<<"|"<< two.ID<<"|"<<two.standing<<"|"<<two.credits<<"|"<<two.gpa<<"|"<<two.dob<<"|"<<two.matriculationDay<<endl;
        return one;
